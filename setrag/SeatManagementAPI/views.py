...
def seats(request):
    ...
    seats_available = Seat.objects.raw("""
                            SELECT t.id,
                                    t.number,
                            	   tp.label,
                                   DATE_FORMAT(FROM_UNIXTIME(t.planned_date/1000), "%%d/%%m/%%Y") as "date_depart",
                                   DATE_FORMAT(FROM_UNIXTIME(t.planned_date/1000), "%%H:%%i") as "heure_depart",
                                   IF(wdr.departure_station = 1, "OWENDO VIRIE", "FRANCEVILLE") as "departure_station",
                                   IF(wdr.arrival_station = 23, "FRANCEVILLE", "OWENDO VIRIE") as "arrival_station",
                            	   count(sbr.seat_occupation_flag) as "seat_occupation_flag"
                            FROM train AS t
